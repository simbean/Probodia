...
def train(args : argparse.ArgumentParser):
    ...
    #set dataloader
    dataloader = setDataloader(csv_file_list = ['F:/food_train_df.csv', 'F:/food_valid_df.csv'], 
    ...
    for epoch in range(args.epoch):  # loop over the dataset multiple times
        ...
        if temp_epoch_F1_score > best_F1_score: 
            ...
            torch.save(model.state_dict(), f'weight/{args.name}.pt')
        torch.save(model.state_dict(), f'weight/model{epoch}.pt')
